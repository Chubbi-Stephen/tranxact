...
import { Shield, Calendar, Lock, CheckCircle2, AlertCircle } from "lucide-react";
import Modal from "./Modal";
import PinModal from "./PinModal";
import SetPinModal from "./SetPinModal";
import { savingsApi, pinApi } from "../../../services/api";
...
const CreateSafelockModal = ({ onClose, onSuccess }) => {
	...
	if (step === "pin") {
		return (
			<PinModal 
				title={`Lock ₦${parseFloat(amount).toLocaleString()}`}
	...
	return (
		<Modal title="Create Safelock" onClose={onClose}>
			<form onSubmit={handlePreSubmit} className="space-y-6 pb-4">
				...
				>
					<Lock size={14} />
					<span>{loading ? "Proceed" : "Enable Safelock"}</span>
				</button>
			</form>
		</Modal>
	);
